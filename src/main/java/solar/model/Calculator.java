...
public class Calculator {
    ...
    public abstract static class Weather {
        ...
        public abstract float getWeatherFactor(int dayNumber);
        ...
        protected static final double[] sunnyDayFactors
                = {0.26, 0.27, 0.31, 0.37, 0.48, 0.49, 0.47, 0.47, 0.47, 0.41, 0.34, 0.31, 0.25, 0.26};

        public static final Weather NOWEATHER = new NoWeather();
        public static final Weather SMOOTHEDWEATHER = new SmoothedWeather();
        public static final Weather RAWWEATHER = new RawWeather();
    }

    private static class NoWeather extends Weather {

        public float getWeatherFactor(int dayNumber) {
            return 1.0f;
        }
    ...
    private static class SmoothedWeather extends Weather {

        private static PolynomialSplineFunction weather;

        public SmoothedWeather() {
            SplineInterpolator li = new SplineInterpolator();
            weather = li.interpolate(months, sunnyDayFactors);
        }

        public float getWeatherFactor(int dayNumber) {
            double month = dayNumber / (DAYSPERYEAR / 12.0);
            return (float) weather.value(month);
        }

        @Override
        public String toString() {
            return "Smoothed weater " + Arrays.toString(sunnyDayFactors);
        }
    }

    /**
     * This is real hack. I dumped the end-of-day figures for the first 6 months
     * of operation, and then re-used some of them to fill in missing months. I
     * now want to scale these numbers so that the monthly average is the same
     * as the smoothed data.
     */
    private static class RawWeather extends Weather {

        double[] scaleFactors = new double[12];
        ...
        public RawWeather() {
            for (int month = 0; month < 12; month++) {
                ...
                for (int i = 1; i <= days; i++) {
                    int dayNumber = dayNumber(month, i);
                    sum += pv1Factors[dayNumber] + pv2Factors[dayNumber] + pv3Factors[dayNumber];
                }
                double mean = sum / days;
                double requiredAverage = sunnyDayFactors[month];
                scaleFactors[month] = requiredAverage / mean;
            }
        }

        public float getWeatherFactor(int dayNumber) {
            int month = Math.min(11, dayNumber / 30);
            double ret = scaleFactors[month] * (pv1Factors[dayNumber] + pv2Factors[dayNumber] + pv3Factors[dayNumber]);
            // sanity check
            if (ret < 0 || ret > 1.0) {
//                throw new IllegalStateException(ret+ "on daynumber "+dayNumber);
            }
            return (float) Math.min(1.0, ret);
        }
